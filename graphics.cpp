...
event_loop (void (*act_on_mousebutton)(float x, float y, t_event_buttonPressed button_info),
			...
			void (*drawscreen) (void)) 
{
// Define this to just generate a picture in postscript format of the 
// screen and return.  Useful for automarking.
#ifdef AUTO_MARKING
...
// For speed, use a fixed size buffer when possible (no allocation).
// Allocate an arbitrary size buffer when necessary only.
